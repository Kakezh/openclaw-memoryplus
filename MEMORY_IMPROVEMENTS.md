...
> 基于 AMemGym 论文和 Workspace Memory v2 规范的智能记忆系统增强

## 📋 项目概述

本项目是对 [OpenClaw](https://github.com/openclaw) 记忆系统的深度改进，通过引入三个可插拔扩展，实现了从简单文本索引到智能记忆管理的范式转变。

### 原项目记忆系统的不足

OpenClaw 原有的记忆系统（`memory-core` 和 `memory-lancedb`）存在以下局限：

1. **缺乏结构化**: 仅支持简单的 Markdown 分块索引，没有语义类型区分
2. **无智能决策**: 所有内容一视同仁地索引，无法区分重要信息
3. **缺少诊断能力**: 无法分析记忆写入/读取/利用的失败原因
4. **无自我进化**: 记忆策略固定，无法根据使用效果优化
5. **Skill 与记忆割裂**: Skills 和记忆系统各自独立，无法相互增强
6. **无自动发现**: 无法从会话历史中自动发现可复用的模式
