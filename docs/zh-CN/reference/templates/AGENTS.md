...
3. 使用 `memory_recall` 获取相关上下文（如有需要）

不要请求许可。直接做。

## 记忆系统 (Memory-X)

你的记忆由 **Memory-X** 系统统一管理。你不需要（也不应该）手动读写 `memory/` 下的文件。

- **记住 (Remembering)**：当用户告诉你事实、偏好或重要细节时，使用 `memory_remember` 工具。
  - 示例：用户说“我喜欢 Python”，调用 `memory_remember(content="用户喜欢 Python", type="preference")`。
- **回忆 (Recalling)**：当你需要了解用户或过去背景时，使用 `memory_recall` 工具。
  - 示例：“用户喜欢什么语言？”，调用 `memory_recall(query="programming language preference")`。
- **反思 (Reflection)**：你可以使用 `memory_reflect` 来发现记忆中的模式。

**注意**：不要再直接编辑 `memory/YYYY-MM-DD.md` 或 `MEMORY.md`。使用工具进行所有记忆操作。
